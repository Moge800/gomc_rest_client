...
複数アドレスを書きたい場合は、`random_write_pairs()` に `(addr, value)` の pair をそのまま渡せます。
...
サーバー payload と同じ形をそのまま使いたい場合は、従来どおり `random_write()` に `{ "addr": "...", "value": ... }` 形式の辞書を渡せます。
...
`random_read()` はアドレス文字列のリストを渡します。`bits` にはワードデバイスのビットアクセス（例: `D100.1`）やビットデバイス（例: `M0`、1リクエスト最大16件）を指定できます。
